...
- **Model access**: The shipped \`.claude/settings.json\` sets **no** model-provider environment, so Claude Code uses the model access you already have and the orchestrator's model and effort are your session's own. Nothing about the framework requires AWS. Bedrock is opt-in: answer \`amazon-bedrock\` at \`aidlc config\` and it writes \`CLAUDE_CODE_USE_BEDROCK\`, your \`AWS_REGION\`/\`AWS_PROFILE\`, and the Fable/Opus/Sonnet/Haiku → global Bedrock model-id mapping into \`settings.json\` for you. You then need Bedrock model access enabled and AWS credentials on the default SDK credential chain. Full setup (model access, IAM, credentials, region) is in \`docs/guide/01-getting-started.md\` § "AWS Bedrock Setup".
