...
4. Based on the user response:
   - **Approve** → `{{INVOKE}} engine orchestrate report --stage <slug> --result approved --user-input "<exact choice>"`. That call emits any missing `STAGE_AWAITING_APPROVAL`, then `GATE_APPROVED` + `STAGE_COMPLETED`, and auto-advances to the next in-scope stage (or completes the workflow on the final stage). No separate `advance` call required.
     - **When `directive.approval_routing === "external"`, do NOT report approved.** This intent answers its gates in a tracker, not here, and the human's "looks good" is a handoff rather than the approval itself. Instead: (a) record the handoff in the tracker — move this stage's issue into its review state and comment with what to review and where the artifacts are; (b) run `{{INVOKE}} engine orchestrate park`. The gate stays open at `[?]`, which is the truthful state: it has been presented and not yet resolved. Say in one line that the stage is with the reviewers and the workflow is parked, then END YOUR TURN. Do not advance, do not start the next stage, and do not report approved on the human's behalf — the approval arrives later as an attested relay carrying `--approval-source`, `--approval-actor`, `--approval-ref`, and `--approval-key` (see "Approving from outside the session" in the interaction-modes guide). Parking is refused under autonomous Construction; that combination cannot arise, because approval routing cannot be set on an autonomous run.
